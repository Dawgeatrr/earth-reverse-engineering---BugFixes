...
const DL_DIR = 'E:/downloaded_files';
...
// -----------------------------------
// QUEUED FILE WRITER UTILITY
// -----------------------------------
function createFileWriter() {
  let promiseChain = Promise.resolve();

  // Appends data to a file, one at a time
  function append(filePath, data) {
    promiseChain = promiseChain.then(() => fs.promises.appendFile(filePath, data));
    return promiseChain;
  }

  // Writes (overwrites) a file, one at a time
  function write(filePath, data) {
    promiseChain = promiseChain.then(() => fs.promises.writeFile(filePath, data));
    return promiseChain;
  }
...
async function run() {
  ...
  if (DUMP_OBJ) {
    ...
    // CHANGED: Initialize a writer queue and store it in the context
    objCtx = initCtxOBJ(objDir);
  }

  let octants = 0;

  // CHANGED: Make nodeDownloaded async so we can await writes
  async function nodeDownloaded(pathString, node, octantsToExclude) {
    console.log('downloaded', pathString);
    if (DUMP_OBJ) {
      // CHANGED: Now we await the async writeNodeOBJ
      await writeNodeOBJ(objCtx, node, pathString, octantsToExclude);
    }
  }
  ...
  const search = initNodeSearch(
    ...
    function nodeFound(pathString) {
      console.log('found', pathString);
      octants++;
    },
    nodeDownloaded // pass in our async callback
  );
        ...
        (async function fn() {
          try {
            ...
            if (results.length === 8) {
              ...
              try {
                // CHANGED: If nodeDownloaded is async, we must await it
                if (nodeDownloaded) {
...
/**************************** export ****************************/
// CHANGED: We store a fileWriter in ctx so all writes get queued
function initCtxOBJ(dir) {
  fs.writeFileSync(path.join(dir, 'model.obj'), `mtllib model.mtl\n`);
  // Create the queued file writer
  const writer = createFileWriter();
  return { objDir: dir, c_v: 0, c_n: 0, c_u: 0, writer };
}

// CHANGED: Make writeNodeOBJ async and use queued writes
async function writeNodeOBJ(ctx, node, nodeName, exclude) {
  for (const [meshIndex, mesh] of Object.entries(node.meshes)) {
    ...
    // Append OBJ data in a queued manner
    await ctx.writer.append(path.join(ctx.objDir, 'model.obj'), obj);

    const { buffer: buf, extension: ext } = decodeTexture(tex);

    // Append MTL data in a queued manner
    const mtlText = `
newmtl ${texName}
...
map_Kd ${texName}.${ext}
`.trim() + "\n";
    await ctx.writer.append(path.join(ctx.objDir, 'model.mtl'), mtlText);

    // Write texture file in a queued manner
    await ctx.writer.write(path.join(ctx.objDir, `${texName}.${ext}`), buf);
  }
}
...
function writeMeshOBJ(ctx, meshName, texName, payload, mesh, exclude) {
  ...
  const consoleBuffer = { log: (s) => (str += s + "\n") };

  consoleBuffer.log(`usemtl ${texName}`);
  consoleBuffer.log(`o planet_${meshName}`);

  consoleBuffer.log("# vertices");
  for (let i = 0; i < vertices.length; i += 8) {
    ...
    consoleBuffer.log(`v ${x} ${y} ${z}`);

    c_v++;
  }

  if (mesh.uvOffsetAndScale) {
    consoleBuffer.log("# UV");
    for (let i = 0; i < vertices.length; i += 8) {
      ...
      if (tex.textureFormat == 6)
        consoleBuffer.log(`vt ${ut} ${1 - vt}`);
      else
        consoleBuffer.log(`vt ${ut} ${vt}`);
      c_u++;
    }
  }

  consoleBuffer.log("# Normals");
  for (let i = 0; i < normals.length; i += 4) {
    ...
    consoleBuffer.log(`vn ${x} ${y} ${z}`);
    c_n++;
  }

  consoleBuffer.log("# faces");
  ...
  for (let i = 0; i < indices.length - 2; i += 1) {
    ...
    const wVal = vertices[a * 8 + 3];

    if (shouldExclude(wVal)) continue;
    const tri = (i & 1) ? [a, c, b] : [a, b, c];
    triangle_groups[wVal] = [].concat(triangle_groups[wVal] || [], [tri]);
  }
  ...
  for (let k in triangle_groups) {
    ...
    for (let t in triangles) {
      ...
      if (mesh.uvOffsetAndScale) {
        consoleBuffer.log(
          `f ${a + _c_v}/${a + _c_u}/${a + _c_n} ` +
          `${b + _c_v}/${b + _c_u}/${b + _c_n} ` +
          `${c + _c_v}/${c + _c_u}/${c + _c_n}`
        );
      } else {
        consoleBuffer.log(`f ${a + _c_v} ${b + _c_v} ${c + _c_v}`);
      }
    }
  }
...
function semaphore(num) {
  ...
  return {
    ...
    signal() {
      concurrent++;
      if (concurrent > 0 && waiting.length > 0) {
        concurrent--;
